...
export const GameInterface: React.FC = () => {
    ...
    const [dialogueState, setDialogueState] = useState(
        DialogueManager.getState().dialogue,
    );
    const [tutorialState, setTutorialState] = useState(
        DialogueManager.getState().tutorial,
    );
    ...
    const [droneExecutionStates, setDroneExecutionStates] = useState<
        Map<string, boolean>
    >(new Map());

    // Challenge Grid state
    const [isOnChallengeGrid, setIsOnChallengeGrid] = useState(false);
    const [showChallengeBlockedMessage, setShowChallengeBlockedMessage] =
        useState(false);
    ...
    // Game store state
    const { entities, activeEntityId, globalResources, isPaused } =
        useGameStore();
    ...
    const shouldBlockModalInteractions = useCallback(() => {
        ...
        if (dialogueState.isActive && shouldHide) {
            console.log(
                "[TUTORIAL] Dialogue hidden - allowing all interactions",
            );
        ...
        const nonDialogueModalsOpen = Array.from(
            globalModalState.openModals,
        ).some((modal) => modal !== "dialogue");
        if (nonDialogueModalsOpen) {
            console.log(
                "[TUTORIAL] Non-dialogue modal open - blocking interactions",
            );
        }
    ...
    const playButtonPosition = { x: window.innerWidth - 220, y: 20 };
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (
                gameMenuModalRef.current &&
                !gameMenuModalRef.current.contains(event.target as Node)
            ) {
    ...
    useEffect(() => {
        ...
        const handleUpgradeModalRequest = () => {
            ...
            if (shouldBlockModalInteractions()) {
                console.log(
                    "Upgrade modal blocked - modal interactions are blocked",
                );
                return;
            }
        ...
        const handleTilesetUpdated = (data: {
        ...
        const handleDroneExecutionCompleted = (data: {
            droneId: string;
            result: any;
        }) => {
            console.log(
                "[DRONE] Execution completed:",
                data.droneId,
                data.result,
            );
            setDroneExecutionStates((prev) => {
        ...
        const handleDroneExecutionFailed = (data: {
            droneId: string;
            error: string;
        }) => {
        ...
        const handleShowLoginModal = () => {
            ...
            if (user && isStudentUser(user)) {
                console.log(
                    "[LOGIN] User already authenticated, starting game",
                );
        ...
        EventBus.on(
            "challenge-movement-blocked",
            handleChallengeMovementBlocked,
        );
        ...
        globalModalState.isAnyModalOpen,
        openModal,
        dialogueState,
        shouldBlockModalInteractions,
        user,
    ]);
    ...
    useEffect(() => {
        ...
        DialogueManager.onStateChange((dialogueState, tutorialState) => {
            ...
            if (
                ...
                !globalModalState.openModals.has("dialogue")
            ) {
                openModal("dialogue");
            } else if (
                !dialogueState.isActive &&
    ...
    const sessionCodeId =
        user && isStudentUser(user) ? user.sessionCodeId : null;
        ...
        const initializeQuests = async () => {
            try {
                ...
                if (sessionCodeId) {
                    try {
                        ...
                        const response = await fetch(
                            `/api/session-codes/quests?sessionCodeId=${encodeURIComponent(sessionCodeId)}`,
                        );

                        // Check response status before parsing
                        if (!response.ok) {
                            console.warn(
                                "[Quest System] Failed to fetch session quests, status:",
                                response.status,
                            );
                        } else {
                            ...
                            if (
                                ...
                                if (validQuests.length > 0) {
                                    ...
                                    console.warn(
                                        "[Quest System] No valid quest paths in session, using all quests",
                                    );
                                }
                            } else {
                                console.log(
                                    "[Quest System] No custom quests for session, loading all quests",
                                );
                            }
                        }
                    } catch (err) {
                        console.error(
                            "[Quest System] Error fetching session quests, using all quests:",
                            err,
                        );
                    }
                }
                ...
                if (
                    ...
                    !questFilesToLoad.includes("quests/game_intro.json")
                ) {
                    questFilesToLoad = [
                        "quests/game_intro.json",
                        ...questFilesToLoad,
                    ...
                    console.log(
                        "[Quest System] Added game_intro for new player",
                    );
                }
                ...
                console.log(
                    "[Quest System] Quests loaded:",
                    questFilesToLoad.length,
                    isSessionSpecificQuests
                        ? "(session-specific, force-unlocked)"
                        : "",
                );
                ...
                if (currentScene !== "ProgrammingGame") {
                    console.log(
                        "[Quest System] Not on ProgrammingGame scene yet, skipping auto-start",
                    );
                    return;
                }
                ...
                if (!hasSeenTutorial) {
                    ...
                    console.log(
                        "[Quest System] New player detected - auto-starting tutorial",
                    );
                    ...
                    setTimeout(() => {
                        ...
                        if (!success) {
                            ...
                            if (typeof getAvailableQuests === "function") {
                                ...
                                if (availableQuests.length > 0) {
                                    success = store.startQuest(
                                        availableQuests[0].id,
                                    );
                                }
                            } else {
                                console.warn(
                                    "[Quest System] getAvailableQuests not available",
                                );
                            }
                        }
                        ...
                        if (success) {
                            ...
                            console.log(
                                "[Quest System] Tutorial started successfully",
                            );
                        } else {
                            console.warn(
                                "[Quest System] Failed to start tutorial",
                            );
                        }
                    }, 500);
                } else {
                    console.log(
                        "[Quest System] Returning player - skipping auto-start",
                    );
                }
            } catch (error) {
                console.error(
                    "[Quest System] Failed to initialize quests:",
                    error,
                );
            }
        };
    ...
    const handleRunCode = () => {
        ...
        const now = Date.now();
        if (now - lastRunCodeCall.current < 500) {
            // 500ms debounce
        ...
        const handleExecutionError = (error: string) => {
            ...
            setTimeout(() => {
                setErrorMessages((prev) =>
                    prev.filter((err) => err.id !== newError.id),
                );
            }, 5000);
        };
            ...
            {showProgrammingInterface && (
                ...
                >
                    ...
                    <SpriteButton
                        ...
                        backgroundSprite="button.png"
                        upFrame={{ x: 432, y: 208, w: 16, h: 16 }}
                        downFrame={{ x: 432, y: 224, w: 16, h: 16 }}
                        scale={3}
                        tooltip="Menu"
                        onClick={() => {
                            if (!shouldBlockModalInteractions()) {
                    ...
                    <SpriteButton
                        ...
                        backgroundSprite="button.png"
                        upFrame={{ x: 544, y: 496, w: 16, h: 16 }}
                        downFrame={{ x: 544, y: 512, w: 16, h: 16 }}
                        scale={3}
                    ...
                    <SpriteButton
                        ...
                        backgroundSprite="button.png"
                        upFrame={{ x: 736, y: 496, w: 16, h: 16 }}
                        downFrame={{ x: 736, y: 512, w: 16, h: 16 }}
                        scale={3}
                        tooltip="Quests"
                        onClick={() => {
                            if (!shouldBlockModalInteractions()) {
                    ...
                    <SpriteButton
                        ...
                        backgroundSprite="button.png"
                        upFrame={{ x: 688, y: 496, w: 16, h: 16 }}
                        downFrame={{ x: 688, y: 512, w: 16, h: 16 }}
                        scale={3}
                        tooltip="My Progress"
                        onClick={() => {
                            if (!shouldBlockModalInteractions()) {
                    ...
                    <SpriteButton
                        ...
                        onClick={() => {
                            if (
                                !isCodeRunning &&
                                activeEntity &&
                                !activeEntity.taskState.isBlocked
                            ) {
                                handlePlantButton();
                            }
                        }}
            ...
            <div style={{
              ...
              <div style={{
                ...
                ({Math.floor(activeEntity.visualPosition?.x ?? activeEntity.position.x)}, {Math.floor(activeEntity.visualPosition?.y ?? activeEntity.position.y)})
                    ...
                    {(() => {
                        // Get all drones
                        const allDrones = Array.from(entities.values()).filter(
                            (e) => e.isDrone,
                        );
                        ...
                        if (displayDrone && displayDrone.isDrone) {
                            const isDroneRunning =
                                droneExecutionStates.get(displayDrone.id) ||
                                false;
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            Position: ({displayDrone.position.x}
                                            , {displayDrone.position.y})
                                        </div>
                                        ...
                                        >
                                            Energy: {displayDrone.stats.energy}/
                                            {displayDrone.stats.maxEnergy}
                                        </div>
                    ...
                    {isOnChallengeGrid && (
                        ...
                        >
                            ...
                            >
                                Arrow keys disabled • Use code to move • No
                                energy cost
                            </div>
                        </div>
                    )}
                    ...
                    {showChallengeBlockedMessage && (
                        ...
                        >
                            🚫 Manual movement blocked! Use code to move on
                            Challenge Grid
                        </div>
                    )}
                    ...
                    {dialogueState.isActive &&
                        DialogueManager.shouldHideDialogue() &&
                        (() => {
                            ...
                            if (
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {currentDialogue.objectives[0].type ===
                                            "movement" &&
                                            currentDialogue.objectives[0]
                                                .directions && (
                                                <div
                                                ...
                                                >
                                                    {currentDialogue.objectives[0].directions!.map(
                                                        (dir) => (
                                                            <span
                                                                key={dir}
                                                                style={{
                                                                    padding:
                                                                        "3px 8px",
                                                                    backgroundColor:
                                                                        tutorialState.movementDirections.has(
                                                                            dir,
                                                                        )
                                                                            ? "#16c60c"
                                                                            : "rgba(0, 0, 0, 0.2)",
                                                                    ...
                                                                    fontSize:
                                                                        "10px",
                                                                    color: tutorialState.movementDirections.has(
                                                                        dir,
                                                                    )
                                                                        ? "#ffffff"
                                                                        : "#210714",
                                            ...
                                            (() => {
                                                ...
                                                const total =
                                                    currentDialogue
                                                        ...
                                                        .length;
                                                const completed =
                                                    currentDialogue.objectives[0].challengePositions.filter(
                                                        (pos) => {
                                                            const grid =
                                                                store.getGridAt(
                                                                    pos,
                        ...
                        >
                            ...
                            <div
                                style={{
                                    position: "relative",
                                    backgroundImage:
                                        "url(/assets/dialogue.png)", // Using title.png as dialogue background
                                    backgroundSize: "contain",
                                    backgroundRepeat: "no-repeat",
                                    backgroundPosition: "center",
                                    width: "100%",
                ...
                >
                    <div
                        style={{
                            ...
                            borderRadius: "12px",
                            boxShadow:
                                "0 8px 0 #210714, 0 12px 20px rgba(0, 0, 0, 0.5)",
                            padding: "30px 35px",
                            display: "flex",
                            flexDirection: "column",
                        }}
                                    ...
                                    const upgrades = [
                                        {
                                            ...
                                            cost: 15,
                                            current:
                                                activeEntity.stats.walkingSpeed,
                                            apply: () => {
                                                const store =
                                                    useGameStore.getState();
                                                store.updateEntity(
                                                    ...
                                                    <div
                                                        ...
                                                        onClick={() => {
                                                            if (canAfford) {
                                                                upgrade.apply();
                                                                console.log(
                                                                    `[UPGRADES] Applied upgrade: ${upgrade.name}`,
                                                                );
                                                            }
                                                        }}
                                                        onMouseEnter={(e) => {
