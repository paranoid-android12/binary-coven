  ...
  if (objectiveProgress.length === 0) {
    return (
      <div className="text-center py-12 px-8 bg-admin-card border border-admin-border rounded-xl shadow-sm">
        <Target className="text-5xl mb-4 opacity-30 text-admin-text-faint" size={48} />
        <p className="text-admin-text-muted text-base m-0">No objective progress data yet</p>
      </div>
    );
  }
    ...
    <div className="w-full">
      ...
      <div className="flex items-center gap-4 mb-6 flex-wrap max-tablet:flex-col max-tablet:items-start">
        ...
        <div className="flex gap-2 flex-wrap max-tablet:w-full">
          <button
            className={`bg-admin-card border border-admin-border rounded-md py-2 px-4 text-admin-text-muted text-[0.85rem] cursor-pointer transition-all duration-300 hover:border-admin-text-faint hover:text-admin-text hover:bg-admin-bg max-tablet:flex-1 max-tablet:text-center ${sortBy === 'completed' ? 'bg-admin-accent border-admin-accent text-white' : ''}`}
            onClick={() => setSortBy('completed')}
          >
            Recently Completed
          </button>
          <button
            className={`bg-admin-card border border-admin-border rounded-md py-2 px-4 text-admin-text-muted text-[0.85rem] cursor-pointer transition-all duration-300 hover:border-admin-text-faint hover:text-admin-text hover:bg-admin-bg max-tablet:flex-1 max-tablet:text-center ${sortBy === 'time' ? 'bg-admin-accent border-admin-accent text-white' : ''}`}
            onClick={() => setSortBy('time')}
          >
            Time Spent
          </button>
          <button
            className={`bg-admin-card border border-admin-border rounded-md py-2 px-4 text-admin-text-muted text-[0.85rem] cursor-pointer transition-all duration-300 hover:border-admin-text-faint hover:text-admin-text hover:bg-admin-bg max-tablet:flex-1 max-tablet:text-center ${sortBy === 'attempts' ? 'bg-admin-accent border-admin-accent text-white' : ''}`}
      ...
      <div className="flex flex-col gap-4">
        {Object.keys(objectivesByQuest).map((questId) => {
          ...
          return (
            <div key={questId} className="bg-admin-card border border-admin-border rounded-xl overflow-hidden transition-all duration-300 shadow-sm">
              <div className="flex items-center gap-4 p-4 cursor-pointer select-none transition-colors duration-300 hover:bg-admin-bg" onClick={() => toggleQuestExpand(questId)}>
                {isExpanded ? (
                ...
                <div className="flex-1">
                  <h3 className="text-xl text-admin-text m-0 mb-2 font-bold max-tablet:text-[1.1rem]">{questTitle}</h3>
              ...
              {isExpanded && (
                <div className="py-0 px-4 pb-4 border-t border-admin-border bg-admin-bg">
                  {objectives.map((objective) => (
                    <div key={objective.id} className="py-4 border-b border-admin-border last:border-b-0">
                      <div className="flex items-start gap-4 max-tablet:flex-col">
                        <div className="bg-admin-accent text-white w-8 h-8 rounded-full flex items-center justify-center text-[0.9rem] font-bold flex-shrink-0 max-tablet:self-start">
                          {objective.objectiveIndex + 1}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-admin-text text-base mb-2 leading-relaxed">
